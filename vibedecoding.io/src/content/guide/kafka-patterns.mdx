...
<Callout type="pattern">
Checkpoint after successful publish. Store the last processed event ID in Postgres, and query for events after that checkpoint on each run.
</Callout>
...
We accept at-least-once delivery (possible duplicates) in exchange for simpler code. The alternative (distributed transactions) adds significant complexity for minimal benefit in a personal system.
</Callout>
